...
def jugar_preguntados(pantalla, fuente_normal, fuente_pequena, reloj, config, sonidos, fondo_img):
    ...
    for pregunta in preguntas:
        ...
        while not respondido and vidas > 0:
            ...
            for i, key in enumerate(comodines_activos):
                if not comodines_activos[key]:
            ...
            for evento in pygame.event.get():
                ...
                if evento.type == pygame.MOUSEBUTTONDOWN and evento.button == 1:
                    ...
                    for key, rect in rects_comodines:
                        if rect.collidepoint(mouse_pos) and not comodines_activos[key]:
                            sonidos['click'].play()
                            if key == 'bomba':
                                opciones = usar_bomba(opciones, correcta)
                                comodines_activos['bomba'] = True
                            elif key == 'pasar': 
                                respondido = usar_pasar()
                                comodines_activos['pasar'] = True
                            elif key == 'doble': 
                                comodines_activos['doble'] = True 
                                doble_chance_activo_para_pregunta = True 
                            elif key == 'x2':
                                comodines_activos['x2'] = True
